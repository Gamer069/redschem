...
async def full_sync(bot: discord.Client):
	...
	channels = [
		discord.utils.get(guild.text_channels, name=i) for i in config.CHANNELS
	]

	hists = {}
	for channel in channels:
		if channel is None:
			logger.warning("Channel {} does not exist, skipping...", channel)
			continue
		hists[channel] = [msg async for msg in channel.history(limit=None)]

	total_messages = sum(len(msgs) for msgs in hists.values())

	log.init_loguru(use_tqdm=True)
